        ...
        for i in self.TO.data_list:
            if i.check_id == check_id:
                if i.wts > self.TS:
                    self.abort()
                    print(
                        "     The record {} has been written by a later transaction. Hence aborting read".format(
                            check_id))
                    return False
                else:
        ...
        for i in self.TO.data_list:
            if i.check_id == check_id:
                if i.rts > self.TS:
                    self.abort()
                    print(
                        "     The record {} has been read by a later transaction. Hence aborting write".format(
                            check_id))
                    return False
                if i.wts > self.TS:
                    pass
